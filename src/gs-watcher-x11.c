...
typedef enum
{
    GS_PRESENCE_STATUS_AVAILABLE = 0,
    GS_PRESENCE_STATUS_INVISIBLE,
    GS_PRESENCE_STATUS_BUSY,
    GS_PRESENCE_STATUS_IDLE,
}GsPresenceStatus; 

#define GS_IDLE_IDLETIME_ID 1
enum
{
    IDLE_CHANGED,
    IDLE_NOTICE_CHANGED,
                         ...
                         GParamSpec       *pspec)
{
	...
	case PROP_STATUS_MESSAGE:
		...
		break;
	case PROP_IDLE_TIMEOUT:
		gs_watcher_set_idle_timeout (self, g_value_get_uint (value));
		break;
	default:
		G_OBJECT_WARN_INVALID_PROPERTY_ID (object, prop_id, pspec);
		break;
            ...
            guint      status)
{
	gboolean res;
	gboolean is_idle;
	gs_debug("enter set_status,status is %d",status);
...
static void
gs_idle_idletime_reset_cb (EggIdletime *idletime, GSWatcher *watcher)
{
	gs_debug ("idletime reset");
	set_status(watcher,GS_PRESENCE_STATUS_AVAILABLE);
}

static void
gs_idle_idletime_alarm_expired_cb (EggIdletime *idletime, guint alarm_id, GSWatcher *watcher)
{
	gs_debug ("idletime alarm id: %i", alarm_id);
	set_status(watcher,GS_PRESENCE_STATUS_IDLE);	
}
...
gs_watcher_init (GSWatcher *watcher)
{
	...
	watcher->priv->idle_timeout = 600000;
	watcher->priv->idletime = egg_idletime_new ();
	g_signal_connect (watcher->priv->idletime, "reset", G_CALLBACK (gs_idle_idletime_reset_cb), watcher);
	g_signal_connect (watcher->priv->idletime, "alarm-expired", G_CALLBACK (gs_idle_idletime_alarm_expired_cb), watcher);
...
gs_watcher_finalize (GObject *object)
{
	...
	egg_idletime_alarm_remove(watcher->priv->idletime,GS_IDLE_IDLETIME_ID);
	g_object_unref(watcher->priv->idletime);
...
void
gs_watcher_set_idle_timeout (GSWatcher *watcher, guint timeout)
{
	g_return_val_if_fail (watcher != NULL, FALSE);
	g_return_val_if_fail (GS_IS_WATCHER (watcher), FALSE);
	gs_debug ("Setting idle timeout: %dms", timeout);
	if (timeout != watcher->priv->idle_timeout) {
		watcher->priv->idle_timeout = timeout;
		if (timeout > 0)
			egg_idletime_alarm_set (watcher->priv->idletime, GS_IDLE_IDLETIME_ID, timeout);
		else
			egg_idletime_alarm_remove (watcher->priv->idletime, GS_IDLE_IDLETIME_ID);
	}
}
